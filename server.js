    ...
    try {
      ...
      try {
        ...
        for (const loan of allLoans.rows) {
          ...
          if (loan.loan_issued_date) {
            const issued = new Date(loan.loan_issued_date);
            const dueDate = new Date(issued.getTime() + term * 24 * 60 * 60 * 1000);
            correctDueDate = dueDate.toISOString().split('T')[0];
          }
          ...
          if (payments.rows.length > 1) {
            ...
            console.log(`   ✅ Removed ${payments.rows.length - 1} duplicate payment(s)`);
          }
...
// Schedule daily run at midnight server time to apply overdue capitalization and status updates
try {
  cron.schedule('0 0 * * *', async () => {
    try {
      console.log('⏰ cron: Running daily due-date capitalization job at', new Date().toISOString());
      await runCheckDueDate();
      console.log('⏰ cron: Completed daily due-date job');
    } catch (err) {
      console.error('⏰ cron: Error running daily due-date job:', err.message);
    }
  }, { scheduled: true });
  console.log('✅ cron: Daily capitalization job scheduled (midnight server time)');
} catch (cronErr) {
...
// Encapsulated due-date & capitalization logic so it can be called from the route and a cron job
async function runCheckDueDate() {
  // Consider both active and already-overdue loans for capitalization
  const result = await pool.query(
    `SELECT * FROM loans
     WHERE due_date < CURRENT_DATE
       AND status IN ('active', 'overdue')
       AND (status = 'overdue' OR extended_this_cycle = false)
       AND (status = 'active' OR last_extended_at IS NULL OR last_extended_at <= CURRENT_TIMESTAMP - INTERVAL '30 days')`
  );
  ...
  for (let loan of loansDue) {
    try {
      ...
      if (monthsOverdue >= 1) {
        ...
        await pool.query(
          `UPDATE loans SET loan_amount = $1, interest_amount = $2, remaining_balance = $3, due_date = $4, status = $5, last_extended_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = $6`,
          [newPrincipal, newInterest, newRemaining, loan.due_date, 'overdue', loan.id]
        );
      ...
      if (loan.status === 'active') {
        ...
        if (totalPaid >= interestAmt) {
          ...
          await pool.query(
            `UPDATE loans SET due_date = $1, interest_amount = $2, remaining_balance = $3, status = $4, last_extended_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP, extended_this_cycle = true, interest_paid_this_cycle = 0 WHERE id = $5`,
          ...
          await pool.query(
            `UPDATE loans SET status = $1, last_extended_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
            ['overdue', loan.id]
          );
        }
      }
...
// Route wrapper for manual trigger
...
app.put('/loans/:loanId', async (req, res) => {
  ...
  try {
    ...
    const updatedLoanResult = await pool.query(updateQuery, [
      ...
      newInterestAmount,
      newTotalPayableAmount,
      newTotalPayableAmount, // remaining_balance set to total_payable_amount
...
app.post('/shift/add-cash', async (req, res) => {
  ...
  try {
    ...
    // Track additional cash separately from opening balance
    // Opening Balance stays fixed; cash added is tracked separately
    const updatedCashAdded = parseFloat(shift.cash_added || 0) + cashAddedAmount;
    ...
    const updateResult = await pool.query(
      `UPDATE shift_management 
       ...
       WHERE id = $3
       RETURNING *`,
      [updatedCashAdded, `[CASH ADDED] $${cashAddedAmount} added - ${notes || 'Bank withdrawal'}`, shift.id]
    );
